...
def build_industrial_graph(vector_store, log_parser, llm_client):
    ...
    # For general queries: gather all available context (manuals + logs)
    builder.add_node("general_context", lambda state: general_context_node(state, vector_store, log_parser))
    builder.add_node("synthesize_response", lambda state: synthesize_response_node(state, llm_client))

    # 2. Define Conditional Router
    def route_intent(
        state: IndustrialAgentState,
    ) -> Literal["retrieve_knowledge", "analyze_logs", "general_context"]:
        intent = state.get("intent", "general")
        if intent == "doc_qa":
            return "retrieve_knowledge"
        elif intent == "log_troubleshoot":
            return "analyze_logs"
        # general intent — pull any available context
    ...
    builder.add_edge("general_context", "synthesize_response")
